...
generate();
function overwriteTrades(rarity, trades, type) {
    const prefix = type ? `${type}_` : '';
    const tradesDir = path_1.join('kubejs', 'data', 'goblintraders', 'trades', prefix + 'goblin_trader');
    const base = path_1.join(tradesDir, 'base', `${rarity}.json`);
    const out = path_1.join(tradesDir, `${rarity}.json`);
    const hasBase = fs_1.existsSync(base);
    const baseTrades = hasBase
        ? JSON.parse(fs_1.readFileSync(base).toString()).trades
        : [];
    const content = {
        replace: true,
        trades: [...trades, ...baseTrades],
    };
    fs_1.writeFileSync(out, JSON.stringify(content, null, 2));
    console.log(`Generated ${rarity} with ${content.trades.length} trades (${baseTrades.length} base trades)`);
}
function tomes(enchantments) {
    return Object.entries(enchantments)
        .map(([ench, level]) => tome(ench, level));
}
function tome(ench, level) {
    return {
