...
export default async function replaceContent() {
  ...
  const replaced: Array<[string, string]> = Object.entries(
    JSON.parse(readFileSync(file).toString())
  );
  const sources = replaced
    .map((a) => a.map((it) => it.split(":")[0]))
  ...
  const assetsRegex = replaced.map(([from, to]) => {
    ...
    const [modTo, idTo] = to.split(":");
    return [
      new RegExp(`/${modFrom}/(.+)/${idFrom}.json$`),
      `/${modTo}/$1/${idTo}.json`,
    ] as [RegExp, string];
  });
  ...
  function replaceTranslations(dir: string, out: string, types: string[]) {
    ...
    const translationsOut = langRegex
      .map(([from, to]) => {
        const replaceKeys = Object.keys(translations).filter((k) =>
          from.test(k)
        );
    ...
    namespaces.forEach((mod) => {
      ...
      const base = existsSync(baseFile)
        ? JSON.parse(readFileSync(baseFile).toString())
        : {};
      writeFileSync(
        outFile,
        JSON.stringify({ ...translationsOut, ...base }, null, 2)
      );
    });
  ...
  function replaceOccurences(dir: string, out: string, replaceable: string[]) {
    const matches = replaceable.reduce(
      (a, s) => [...a, ...glob.sync(s, { cwd: dir })],
      [] as string[]
    );

    matches.forEach((s) => replace(join(dir, s), join(out, s)));
